...
contract PIScaledPerSecondCalculatorTest is DSTest {
    ...
    function test_two_small_positive_deviations() public {
        ...
        assertEq(uint(calculator.lut()), now);
        assertEq(calculator.pdc(), -1889999999999999999999999998290000000000000000000000000000);
        assertEq(oracleRelayer.redemptionPrice(), 1);
    ...
    function test_normalized_pi_result() public {
        ...
        Kp = Kp / 4 / int(calculator.ips() * 24);
        Ki = Ki / 4 / int(calculator.ips() ** 2) / 24;
    ...
    function test_correct_proportional_calculation() public {
        ...
        assertEq(Ki, 0);
        assertEq(Kp * 4 * int(calculator.ips()) * 96, 999999999999129600);
        ...
        (int gainAdjustedP,) = calculator.getGainAdjustedTerms(-int(0.025E27), int(0));
        assertEq(gainAdjustedP, -18084490740725000000);
        assertEq(gainAdjustedP * int(96) * int(calculator.ips()) * int(4), -24999999999978240000000000);
        ...
        (gainAdjustedP, ) = calculator.getGainAdjustedTerms(int(0.025E27), int(0));
        assertEq(gainAdjustedP, 18084490740725000000);
        assertEq(gainAdjustedP * int(96) * int(calculator.ips()) * int(4), 24999999999978240000000000);
    }
